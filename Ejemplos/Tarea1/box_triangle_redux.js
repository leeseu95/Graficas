...
function createSphere(gl, radius)
{
    ...
    for (let i = 0; i < iters; i++) {
        rads += (4.71239/iters);
        temp = 0.5*Math.cos(rads);
        verts.push(temp);
        temp = 0.5*Math.sin(rads);
        verts.push(temp);
        verts.push(0);
    }
    console.log(verts);
    ...
    var sphere = {buffer:vertexBuffer, vertSize:3, nVerts:iters, primtype:gl.TRIANGLE_FAN};
    return sphere;
}        